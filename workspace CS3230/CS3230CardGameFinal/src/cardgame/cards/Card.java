...
public class Card {
	...
	public Card(Numbers num, Suits suit, int startPosX, int startPosY, int rotation){
		...
		// add click listener
		/*
		 * uncomment this part if you want to be able to click on the cards to flip them
		 * 
	...
	public void flip(){
		...
		st1.setOnFinished(setImageToReverse());
	...
	private EventHandler<ActionEvent> setImageToReverse() {
    	return (event) -> {
    		if(facingFront){
    			this.face.setImage(new Image(this.sourcePath + this.frontImageString));
    		}
    		else{
    			this.face.setImage(new Image(this.sourcePath + this.backImageString));
    		}
    	};
	}
	
	public void moveToRelative(double x, double y, int time, int delay){
	...
	public void moveTo(double x, double y, int time, int delay, boolean flip, int angle){
		...
		tt.setToX(x - this.face.getX());
		tt.setToY(y - this.face.getY());
		tt.setCycleCount(1);
		tt.setAutoReverse(true);
