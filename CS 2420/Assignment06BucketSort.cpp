...
void placeIntoBuckets() {
  //TODO: Put the values into the appropriate buckets.
	unsigned long maxNumber = 4294967295; // this is 2^32
	int percentage = 0;
	int bucketToUse = 0;
	for(long i=0; i<listSize; i++){
		percentage = (int) (((double) list[i] / (double) maxNumber) * listSize);
		bucketToUse = listSize / globalBuckets->getNumBuckets();

		bucketToUse = percentage / bucketToUse;

		globalBuckets->addItem(bucketToUse, list[i]);
	}    
}
...
void sortEachBucket() {
  ...
  //TODO: Sort each individual bucket
	for(int i=0; i<globalBuckets->getNumBuckets(); i++){
